...
>
> 我们的方案：在函数签名上直接声明能力（`_IO_Async_Fallible`），让调用处自解释——不用跳转到定义就知道这个函数会做 IO、是异步的、可能失败。这对人来说太繁琐（谁愿意给每个函数加后缀？），但对 LLM 零成本。配合 Parse don't validate 等类型约束手段，模块间的信任建立在类型上而非文档上。
>
> 这不是引入新框架（如 Effect-TS），而是**让现有代码自己说话**——能力标注降低理解成本，评分系统驱动结构优化。
...
| 019 | [.fts/.tts 工作空间协议](019-fts-workspace-protocol.md) | 废弃 — 定义新语言格式弊大于利，详见 [wiki 词条](wiki/wiki-docs/file-ts/README.md) |
