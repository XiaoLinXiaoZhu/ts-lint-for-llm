...
**状态：归档。基础原则已确认，与 E（Fallible）能力的关系在 012 中有新发展。**

Result + 可辨识联合错误建模是已确认的基础做法，不需要继续发散。

与后续实践的关联：012 揭示了 E（Fallible）是所有能力中"最特殊的"——它是唯一可以通过包装消除的能力。这给 003 的错误建模增加了一个新视角：错误处理方式（throw vs Result vs Parse don't validate）直接影响 E 在调用链上的传播范围，进而影响评分。

具体来说：003 讨论的是"错误应该怎么表示"，012 讨论的是"错误表示的选择如何影响系统的能力负担"。两者互补但各有侧重，003 本身可归档。
