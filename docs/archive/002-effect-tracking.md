...
**状态：已被 011 + 012 继承和具体化。可归档。**

这篇提出的能力字母表是整个后续实践的起点。002 中的概念已经被完整落地：

- 011 实现了 JSDoc `@capability` + ESLint `no-escalation` 规则 + 外部包补丁文件
- 012 实现了逐行加权评分模型，用实验数据验证了"按能力隔离拆分"的量化收益

002 本身可以归档——它的核心主张（副作用应该在签名中显式标记、权限传播规则、好函数判定标准）已经全部在原型中可运行。后续工作在 011/012 的待探索清单上，不需要回到 002 发散。

唯一值得注意的演化：002 用的是单字母后缀（`_AEI`），011 最终选择了完整单词后缀（`_IO_Async_Fallible`）和 JSDoc 双轨制。这个决策已经固化，不需要重新讨论。

## 我的看法2

这里需要看看新的文档有没有解释清楚，因为我们选择完整单词后缀的原因是：
- 对于人类它看着繁琐
- 对于模型来说它是自解释的
- 模型写的时候，全大写字符和完整单词差别不大，也就多2、3个token