 ...
 * .cap.ts 外部能力声明文件解析器
 *
 * 用户通过 .cap.ts 文件声明外部库函数的 IO/Impure 能力。
 */
...
const VALID_CAPS = new Set<string>(CAPABILITIES);
...
function extractCaps(node: Node): Capability[] | null {
    ...
    if (match) {
      ...
      if (match[1]) {
        for (const word of match[1].trim().replace(/\*\/.*$/, "").trim().split(/[\s,]+/)) {
          if (VALID_CAPS.has(word)) caps.push(word as Capability);
        }
      }
      return caps;
    }
